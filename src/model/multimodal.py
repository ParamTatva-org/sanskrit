...
class MultiModalParamtatva(nn.Module):
    ...
    def __init__(
        ...
        if not config.add_cross_attention:
            raise ValueError(
                "ModelConfig must have add_cross_attention=True for MultiModal model"
            )
        ...
        if vision_backbone:
            ...
            self.vision_projection = nn.Linear(
                self.vision_encoder.feature_dim, config.hidden_dim
            )

        if video_backbone:
            self.video_encoder = VideoEncoder(backbone_name=video_backbone)
            self.video_projection = nn.Linear(
                self.video_encoder.feature_dim, config.hidden_dim
            )
        ...
        self.decoder = ParamtatvaTransformer(
            config, ptk_graph, token_to_id=token_to_id, id_to_token=id_to_token
        )
        ...
        if video_values is not None and self.video_encoder is not None:
            ...
            if modal_indices is not None and visual_batch_size < batch_size:
                ...
                encoder_attention_mask = torch.zeros(
                    batch_size, visual_seq_len, dtype=torch.long, device=device
                )
    ...
    def generate_caption(
        ...
        with torch.no_grad():
            ...
            input_ids = torch.tensor(
                [[tokenizer.token_to_id[tokenizer.BOS_TOKEN]]], device=device
            )
...
if __name__ == "__main__":
    ...
    print(
        f"MultiModal Model created with {sum(p.numel() for p in model.parameters()):,} parameters"  # noqa: E231
    )
