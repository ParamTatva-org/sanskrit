...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, config: ModelConfig, ptk_graph: Optional[Any]):
        ...
        if config.use_pratyahara_bias:
            self.pratyahara_bias = PratyaharaAttentionBias(
                ptk_graph, config.num_heads, vocab_size=config.vocab_size
            )
        else:
    ...
    def forward(
        ...
        if self.cross_attention_norm is not None and encoder_hidden_states is not None:
            ...
            k = k.view(
                batch_size, enc_seq_len, self.num_heads, self.head_dim
            ).transpose(1, 2)
            v = v.view(
                batch_size, enc_seq_len, self.num_heads, self.head_dim
            ).transpose(1, 2)
...
class ParamtatvaTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [TransformerBlock(config, ptk_graph) for _ in range(config.num_layers)]
        )
...
if __name__ == "__main__":
    ...
    print(f"Model created with {model.get_num_params():,} parameters")  # noqa: E231
    ...
    print(f"Loss: {loss.item():.4f}")  # noqa: E231
