...
class ParamtatvaEmbedding(nn.Module):
    ...
    def __init__(
        ...
        if use_graph_features:
            ...
            self.register_buffer('_sutra_lookup', torch.zeros(self.vocab_size, dtype=torch.long))
            self.register_buffer('_position_lookup', torch.zeros(self.vocab_size, dtype=torch.long))
